...
impl Plugin for LodPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<LodState>().add_systems(
            Update,
            (
                update_frustum,
                update_lod_requests,
...
/// Update LOD requests based on camera position.
...
fn update_lod_requests(
    ...
    // Clone to avoid borrow conflicts when mutating lod_state later.
    ...
    for bulk_path in bulk_paths {
        ...
        for node_meta in &bulk.nodes {
            ...
            // Skip already loaded or loading nodes.
            ...
            // Check LOD - compute node center from OBB.
        ...
        for child_path in &bulk.child_bulk_paths {
            ...
            // Skip already loaded or loading bulks.
            ...
            // Check if any area of this bulk might be visible.
            // For now, always load child bulks if parent is loaded.
...
fn cull_meshes(
    ...
    for (transform, mut visibility, _marker) in &mut query {
        ...
        // Create a small OBB around the mesh position.
